...
<div class="card-body">
  ...
  <div class="pt-4 pb-2">
    <h5 class="card-title text-center pb-0 fs-4">Edit an Site Category</h5>
    <p class="text-center small">Enter Site Category details to create It</p>
  </div>

  <form class="row g-3 needs-validation"  method="POST" action="{{ route('update.sitecategory') }}" enctype="multipart/form-data" novalidate>  
        @csrf
        <input type="hidden" name="sitecategory_id" value="{{$sitecategory->id}}">
    

    <div class="col-12">
      <label for="yourEmail" class="form-label">Category Name</label>
        <input type="text" name="category_name" class="form-control" id="yourUsername" value = "{{$sitecategory->category_name}}" required>
    ...
    <div class="col-12">
      ...
      <div class="input-group has-validation">
        ...
        <textarea id="projectinput9" rows="5" class="form-control @error('description') is-invalid @enderror" placeholder="Reject Reason"
                           name="description"  required>{{$sitecategory->description}}</textarea>
                          @error('description')
    ...
    <div class="col-12">
      <button class="btn btn-primary w-100" type="submit">Edit Site Category</button>
    </div>
  </form>
